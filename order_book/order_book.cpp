...
struct Order { // use stuct for data object
    ...
    auto operator<=>(const Order &) const = default; // TODO: to order Order by price and timestamp to allow for natural
                                                     // order in the order_book's priority queue
};
// for printing order
std::ostream &operator<<(std::ostream &os, const Order &order) {
    os << "Order{id=" << order.id << ", symbol=" << order.symbol << ", side=" << to_string(order.side)
       << ", quantity=" << order.quantity << ", price=" << order.price << ", timestamp=" << order.timestamp << "}";
    return os;
}
